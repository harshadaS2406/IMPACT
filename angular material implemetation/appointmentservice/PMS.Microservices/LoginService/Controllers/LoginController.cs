        ...
        private static readonly string[] Summaries = new[]
       {
        ...
        public async Task<IActionResult> Login([FromBody] UserLogin objUsers)
        {
            try
            {
                ...
                {
                    userManagerResult.AccessFailedCount= userManagerResult.AccessFailedCount + 1;
                    await userManager.UpdateAsync(userManagerResult);
                    if (userManagerResult.AccessFailedCount <= 3)
                    {
                        response = new Response
                        {
                            ResponseCode = Codes.InvalidCredentials,
                            ResponseInfo = 3 - userManagerResult.AccessFailedCount
                        };

                        return Ok(response);
                    }
                    else if (userManagerResult.AccessFailedCount == 3)
        ...
        public string GenerateToken(ApplicationUser applicationUser)
        {
            ...
            var securityKey = new SymmetricSecurityKey(Convert.FromBase64String(_config["Jwt:Key"].ToString()));
            securityKey.KeyId = "hspisces15";

            var credentials = new SigningCredentials(securityKey, SecurityAlgorithms.HmacSha256);

            //    varSigningCredentials = new SigningCredentials(
            //new SymmetricSecurityKey(securityKey) { KeyId = "Your Key Id" },
            ...
            var token = new JwtSecurityToken(_config["Jwt:Issuer"], _config["Jwt:Audience"], claimsDetail, expires: System.DateTime.Now.AddMinutes(15), signingCredentials: credentials);
